...
type SplitOption = 'characters' | 'words' | 'lines' | string;
...
const GlobalSpotlight: React.FC<GlobalSpotlightProps> = ({
  ...
  const spotlightRef = useRef<Maybe<HTMLDivElement>>(null);
  ...
  useEffect(() => {
    ...
    const handleMouseLeave = () => {
      ...
      gridRef.current?.querySelectorAll<HTMLElement>('.magic-card').forEach(card => {
        card.style.setProperty('--magic-glow-intensity', '0');
      });
...
const BentoCardGrid: React.FC<{
  ...
  <div className="magic-grid magic-bento-section" ref={gridRef}>
    {children}
  </div>
);
      ...
      <BentoCardGrid gridRef={gridRef}>
        {cardData.map((card, index) => {
          ...
          return (
            <div
              key={card.label ?? index}
              className={`${baseClassName.trim()} magic-particle-container`}
